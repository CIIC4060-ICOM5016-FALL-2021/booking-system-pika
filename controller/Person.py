...
def get_all_persons(limit: int = 75):
    method = PersonDAO()
...
def update_person(json: dict):
    p_id = json['p_id']
    p_fname: str = json['p_fname']
    p_lname: str = json['p_lname']
    p_phone: str = json['p_phone']
    p_gender = json['p_gender']
    method = PersonDAO()
    exist = method.check_if_person_exists(p_id)
    updated_info = False
    if type(p_gender) == int:
        updated_info = method.update_person(p_id, p_fname, p_lname, p_phone, p_gender)
    elif type(p_gender) == str:
        for key, value in method.genders.items():
            if value == str(p_gender).lower():
                updated_info = method.update_person(p_id, p_fname, p_lname, p_phone, key)
                break
