...
class RoomDAO(object):
    ...
    def check_if_room_exists(self, r_id):
    ...
    def find_available_rooms(self, st_dt, et_dt):
        cursor = self.conn.cursor()
        query = 'select r_id, r_name, r_type from room ' \
                'where room.r_id not in (select distinct r_id from room inner join (select ' \
    ...
    def create_new_room(self, r_name: str, r_building: str, r_dept: str, r_type: int) -> int:
        cursor = self.conn.cursor()
        query = "insert into room (r_name, r_building, r_dept, r_type)  " \
                "values (%s, %s, %s, %s) returning r_id;"
        cursor.execute(query, (r_name, r_building, r_dept, r_type,))
    ...
    def update_room(self, r_id: int, r_new_name: str, new_r_building: str, new_r_dept: str, new_r_type: int) -> bool:
        cursor = self.conn.cursor()
        query = "update room set r_name = %s, r_building = %s, r_dept = %s, r_type = %s " \
                "where r_id = %s;"
        cursor.execute(query, (r_new_name, new_r_building, new_r_dept, new_r_type, r_id,))
        self.conn.commit()
        cursor.close()
        return True
